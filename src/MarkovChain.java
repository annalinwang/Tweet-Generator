...
public class MarkovChain implements Iterator<String> {
	...
	void addBigram(String first, String second) throws IllegalArgumentException {
		if (first.equals(null)) {
			...
			if (!chain.containsKey(first)) {
				ProbabilityDistribution<String> myDis = new ProbabilityDistribution<String>();
				chain.put(first, myDis);
			}
			for (Map.Entry<String, ProbabilityDistribution<String>> entry : chain.entrySet()) {
				if (entry.getKey().contentEquals(first)) {
					entry.getValue().record(second);
				}
			}
		}
	}
	 ...
	 */
	public void train(Iterator<String> sentence) throws IllegalArgumentException {
		if (sentence == null) {
			throw new IllegalArgumentException();
		}
		String first = null;
	...
	public String next() throws NoSuchElementException {
		...
		} else if (chain.get(nextWord) == null) {
			nextWord = null;
		} else {
			nextWord = chain.get(nextWord).pick(ng);
		}
		return res;
	}
