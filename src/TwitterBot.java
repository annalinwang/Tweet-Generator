...
public class TwitterBot {
	 ...
	 *                                  MAX_TWEET_LENGTH
	 */
	public String generateTweet(int length) throws IllegalArgumentException {
		...
		boolean check = cLen < length - 1;
		while (check) {
			if (!mc.hasNext()) {
				myString += randomPunctuation() + " ";
				mc.reset();
				cLen += 2;
			} else {
				String next = mc.next();
				if (!mc.hasNext() || (cLen + next.length() + 1 >= length)) {
					myString += next;
					cLen += next.length();
				} else {
					myString += (next + " ");
		...
		return myString += randomPunctuation();
	}
	...
	public static void main(String args[]) {
		...
		/*
		 * You can also write randomly genrated tweets to a file by uncommenting the
		 * following code:
		 */
